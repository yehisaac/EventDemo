...
# 各階段思考與規劃
**Day 1**
- 由於之前沒Vibe Coding經驗，這時先跟gemini&gpt聊天試著產生one single page草稿測試各AI對於程式的理解與產生，雖然都是用免費版但以程式碼來看此階段Gemini更勝一籌，至於idea的討論隨著討論我認為兩邊給的資訊都是有用的主要就看專案應該往哪邊走。
- 此時主要著重在工具的選擇以及未來開發方式，這時了解到以免費版的能力無法提升開發速度，必須讓AI介入修改檔案與產生相關文件。

**Day 2**
- 選擇使用Claude的原因很簡單他有七天的試用，之後開始將Day1討論的規格直接貼給Claude請他產生，這時的初步規劃的想法只有線上簽到與線下，我認為既然是Vibe Coding就是要先有初步想法後面想要甚麼再慢慢掛上即可。
- 初版出來後進行測試。
- 增加了線下簽到功能。

**Day 3**
- 可能是第一次init所以token消耗飛快，立即跟AI討論是不是沒有拆檔案分架構的原因，AI普遍回答要拆架構出來，所以這天進行了模組化，要求是以減少token為目標規劃模組，模組化後AI DEBUG的能力有稍微下降，較常需要人為介入。
- 增加了Task獲得積分功能。
- 此時突然想測試各AI工具利用相同的MD檔會產生甚麼結果，所以也用了額度在產生相關的MD檔案，主要分為以下幾類

### MD檔案內容

| 檔案 | 職責 |
|------|------|
| event-system-spec.md | 技術規格文件(工程師看) |
| README-MODULAR.md | 模組化架構文件(專案架構) |
| README.md | 一般規格文件(給PM看) |
| CLAUDE.md | 請AI產生給其他AI看的內容 |

**Day 4**
- 畫面偏醜，下了簡單色調的指令請他重構整了設計風格。
- 增加unit Test，正常系統都會有CI/CD想說有基本unit Test可以減少後續AI改壞程式的可能。
- 增加QRCODE簽到制度，報名候補制度機制。

**Day 5**
- 測試&bugfix。
- Weekly limits 使用量為49%

# 下階段可以做甚麼(程式架構方面)
- Infra as code的建立，結合真正的ci/cd or Dockerize 試看看在DevOps領域較果以及能否無腦整合其他雲服務
- 現行有許多軟體開發方法論如DDD、TDD、BDD等等複雜的架構方式哪種對AI更容易判讀
- 目前的小專案問沒幾次上下文的token量就會超過限制，如果是大型專案程式碼是否吃得下來且能理解
- AI與DB之間的溝通能力，開發時能否連CRUD也透過AI處理
